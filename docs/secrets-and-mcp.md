...
|----------|-----------|----------------------|
| `plynx run <project> <workflow>` | Runtime injection | PilotLynx injects secrets into the Agent SDK subprocess directly |
...
- A PilotLynx workspace (created with `plynx init`)
- At least one project (created with `plynx project create` or `plynx project add`)
...
```bash
plynx env my-web-app
# ANTHROPIC_API_KEY="sk-ant-..."
# GITHUB_TOKEN="ghp_..."
# DATABASE_URL="postgres://..."
# SLACK_URL="https://hooks.slack.com/..."

plynx env my-cli-tool
# ANTHROPIC_API_KEY="sk-ant-..."
# GITHUB_TOKEN="ghp_..."
```
...
```bash
plynx env my-web-app --export    # export KEY=value (shell)
plynx env my-web-app --json      # {"KEY": "value"}
plynx env my-web-app --envrc     # .envrc format with PILOTLYNX_ROOT
```

### Step 4: Link the project for direct access

```bash
plynx link my-web-app --direnv
```

This does two things:

1. **Updates `.claude/settings.json`** in the project — adds `PILOTLYNX_ROOT` so `plynx` CLI commands work from inside the project directory.
...
# Re-run: plynx link my-web-app --direnv
...
When you adopt an existing project with `plynx project add`, secrets are **automatically detected and consolidated**.

### What happens

During `plynx project add`, the CLI scans the project for secrets in two places:
...
- Backs up the project `.env` to `.env.plynx-backup`
...
The project's original `.env` is preserved as `.env.plynx-backup` in case you need to reference it. Once you've verified everything works, you can delete the backup.

Link the project for direct access with direnv:

```bash
plynx link my-existing-app --direnv
...
secrets-access.yaml                Per-project allowlist (committed, reviewable)
       |
       v
plynx link --direnv                Generates .envrc with only allowed secrets
       |
       v
...
- `plynx env` lets you audit exactly what each project can see at any time.
